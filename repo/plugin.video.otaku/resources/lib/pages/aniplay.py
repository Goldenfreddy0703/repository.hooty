...
class sources(BrowserBase):
    ...
    def get_sources(self, anilist_id, episode, get_backup):
        ...
        if items:
            ...
            ids = [item.get("id") for item in items if any(f"https://anilist.co/anime/{anilist_id}" in website["url"] for website in item.get("listWebsites", []))]
    ...
    def _process_al(self, ids, title, episode):
        ...
        for id in ids: 
            ...
            if (items):
                ...
                season_id = next(season["id"] for season in sorted(items, key=lambda x: x["episodeStart"]) if int(episode) >= int(season["episodeStart"]))
